...
public class screen extends JPanel implements MouseListener, MouseMotionListener{
    ...
    public screen(int x, int y){
        ...
        height = new int[x - 1];
        render = new LinkedBlockingQueue<Pixel>();
...
public void clear(){
    ...
    public boolean addPixel(int num){
        try {
            sem.acquire();
    ...
    public int getPixels(boolean interupted){
        try {
            sem.acquire();
        } catch (InterruptedException e) {
            interupted = true;
